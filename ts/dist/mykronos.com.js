...
var minutesDiff = (s, e) => (e.getTime() - s.getTime()) / 1e3 / 60;
...
var iCalEvent = (e, name, email) => {
  ...
  const breakTime = breaks.map((b) => minutesDiff(new Date(b.startDateTimeUTC), new Date(b.endDateTimeUTC))).reduce((p, c) => p + c, 0);
  const breakTimes = breaks.map((b) => time(new Date(b.startDateTimeUTC))).join(", ");
